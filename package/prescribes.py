...
class Prescribes(Resource):
    ...
    def get(self):
        ...
        prescribes = conn.execute("SELECT prescribes.doc_id, doctor.doc_first_name, doctor.doc_last_name, prescribes.pat_id, patient.pat_first_name, patient.pat_last_name, prescribes.code, prescribes.p_date, prescribes.app_id, dose FROM prescribes INNER JOIN doctor ON prescribes.doc_id = doctor.doc_id INNER JOIN patient ON prescribes.pat_id = patient.pat_id").fetchall()
        return prescribes
    ...
    def post(self):
        ...
        code = prescribes['code']
        p_date = prescribes['p_date']
        app_id = prescribes['app_id']
        dose = prescribes['dose']
        conn.execute('''INSERT INTO prescribes(doc_id, pat_id, code, p_date, app_id, dose) VALUES(?,?,?,?,?,?)''', (doc_id, pat_id,code, p_date, app_id, dose))
        conn.commit()
        return prescribes
...
class Prescribe(Resource):
    ...
    def put(self,doc_id):
        ...
        app_id = prescribes['app_id']
        dose = prescribes['dose']
        conn.execute("UPDATE prescribes SET doc_id=?,pat_id=?,code=?,p_date=?,app_id=?,dose=?, WHERE doc_id=?", (doc_id, pat_id, code, p_date, app_id, dose, doc_id))
        conn.commit()
        return prescribes